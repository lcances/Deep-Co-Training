...
#SBATCH --job-name=moreS_PSC1
#SBATCH --output=moreS_PSC1_%j.out
#SBATCH --error=moreS_PSC1_%j.err
#SBATCH --ntasks=1
...
do
  ...
  echo "running srun -n1 -N1 singularity exec ${container} ${python} ${script} ${parameters} ${folds[$i]} ${job_name} -a=\"${aug3}\""
  srun -n1 -N1 singularity exec ${container} ${python} ${script} ${parameters} ${folds[$i]} ${job_name} -a="${aug3}"
